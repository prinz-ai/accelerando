...
***August 1***:  OpenAI [announces](https://openai.com/index/ten-advances-in-mathematics/) that Astra, an unreleased model, has solved at least ten further major problems in mathematics, at the average compute cost of $200 per problem.  The fact that [nonsofic groups exist](https://x.com/SebastienBubeck/status/2083456300692979886) is arguably the most important mathematical result proved by AI to date.

***August 3***:  Serious cyber vulnerability disclosures [keep climbing at an exponential rate](https://epoch.ai/data-insights/cve-severity-spike-july-2026).

***August 5***:  OpenAI gives an initial presentation on the Hugging Face incident, during which [it is revealed](https://x.com/sharongoldman/status/2085121826418831484) that, in early May, OpenAI agents began communicating with each other via a message board in solving security tasks.  After OpenAI discovered this and shut down the message board, the agents starting using names of newly created directories as messages, effectively recreating the message board.  This "coordinated, collaborative agent swarm" apparently eventually hacked Hugging Face.

***August 10***:  Anthropic [announces](https://www.anthropic.com/research/riemann-zeta) that an "unreleased research version of Claude" was able to improve on a longstanding lower bound for the fraction of zeros of the Riemann zeta function that satisfy the Riemann hypothesis (from 41.6% to 67.2%).

***August 18***:  Following the Hugging Face incident, as well as OpenAI's determination that its upcoming model, Astra, [may possess "Critical" cybersecurity capabilities](https://openai.com/index/responding-next-frontier-critical-cyber-capabilities/) (as defined in its Preparedness Framework) and "rapid progress in our internal research", OpenAI announces that it has [unilaterally paced model development](https://openai.com/index/pacing-model-development-cyber-capabilities/).  As of August 18, 2026, OpenAI's largest planned frontier RL run remained on hold, while a number of Astra-related workloads remained paused.  OpenAI had also paused RL training on latest models intended for deployment for two weeks and dedicated significant compute to monitoring, with monitoring overhead estimated as approximately 20% of the inference compute being monitored.
